...
//use ring::signature::{verify, ECDSA_P256_SHA256_ASN1 as ALG};
...
use get_if_addrs::{get_if_addrs as ifaces, IfAddr};
...
fn ping(sock: &UdpSocket, addr: String) {
  ...
  match sock.send_to(&data, &addr) {
    Ok(n) => if n == 8 { println!("pinged {} with seqid {}", addr, seq); },
...
impl Dormouse {
  fn new() -> Dormouse {
    ...
    let cfgv = match Dormouse::read_cfg() {
      Ok(res) => res,
      Err(err) => panic!("Error reading config: {:?}", err)
    };

    let (key, ping_port, http_port, _t_vid_file) = match cfgv.get("config") {
      Some(v) => match v.as_table() {
        Some(keys) =>
          (
            match keys.get("privkey") {
            ...
            match keys.get("ping_port") {
              Some(v) => match v.as_integer() {
                Some(v) => v,
                None => panic!("Error reading config: ping_port isn't an integer!")
              },
              None => 1832
            },

            match keys.get("http_port") {
              Some(v) => match v.as_integer() {
                Some(v) => v,
                None => panic!("Error reading config: http_port isn't an integer!")
              },
              None => 80
            },

            match keys.get("_t_verif_pubkey") {
              Some(v) => match v.as_str() {
                Some(v) => v,
                None => panic!("Error reading config: _t_verif_pubkey isn't a string")
              },
              None => ""
            }.to_string()
          ),
        None => panic!("'key' should be a table")
      },
      None => panic!("Error reading config: section 'config' does not exist")
    };

    let cfg = Config{key, ping_port, http_port, _t_vid_file};
  ...
  fn spawn_pinger(&self) {
    ...
    // buuuuuuuut I'm lazy
    let pinger = UdpSocket::bind(format!("0.0.0.0:{}", self.cfg.ping_port)).unwrap();
    pinger.set_broadcast(true).unwrap();
    let ping_port = self.cfg.ping_port;
          ...
          if iface.name.starts_with("p2p") {
            match iface.addr {
              IfAddr::V4(addr) => match addr.broadcast {
                Some(brip) => {
                  unsafe { rand.fill(std::mem::transmute::<&mut i64, &mut [u8;8]>(&mut SEQID)).unwrap(); };
                  ping(&pinger, format!("{}:{}", brip, ping_port));
                },
                _ => unsafe { SEQID = 0; }
              },
              _ => unsafe { SEQID = 0; }
  ...
  fn server_test(&self) {
    ...
    let mut vid_file = File::open(&self.cfg._t_vid_file).unwrap();
    let mut contents = String::new();
    vid_file.read_to_string(&mut contents).unwrap();
    let vid = PubKey::from_pem(contents.as_bytes()).unwrap().to_point_bytes().unwrap();
    ...
    /*for byte in sgn_prf.write_to_bytes().unwrap() {
      print!("{:X}", byte);
    }*/
    println!("");
    let mut resp =
  ...
  fn gen_proof(&self, uid: &[u8], unonce: &[u8], nonce: &[u8], time: u64) -> Result<Vec<u8>, String> {
    ...
    /*for byte in sgn_msg.write_to_bytes().unwrap().as_slice() {
      print!("{:X}", byte);
    }*/
    println!("");
    let resp =
    ...
    Ok(unsafe {std::mem::transmute(vault_key)})
  }
}
