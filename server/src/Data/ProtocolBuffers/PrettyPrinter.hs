...
module Data.ProtocolBuffers.PrettyPrinter where
...
-- Expansion of the above to avoid using the type family on the RHS
instance (KnownNat n, ProtoTypeName c) => GPrettyProtoSel (K1 i (Field n (RequiredField (Always (Value c))))) where
...
instance (KnownNat n, ProtoTypeName c) => GPrettyProtoSel (K1 i (Field n (RequiredField (Always (Enumeration c))))) where
  gprettyProtoSel _ selName' = text "required" <+> protoTypeName (Proxy :: Proxy c) <+> text selName'' <+> text "=" <+> text (show (natVal (Proxy :: Proxy n))) <> char ';' where
    selName'' = if null selName' then "field" ++ show (natVal (Proxy :: Proxy n)) else selName'

instance (KnownNat n, ProtoTypeName c, Dataname (Rep c)) => GPrettyProtoSel (K1 i (Field n (RequiredField (Always (Message c))))) where
  gprettyProtoSel _ selName' = protoTypeName (Proxy :: Proxy c) $+$ text "required" <+> dataname (Proxy :: Proxy (Rep c)) <+> text selName'' <+> text "=" <+> text (show (natVal (Proxy :: Proxy n))) <> char ';' where
    selName'' = if null selName' then "field" ++ show (natVal (Proxy :: Proxy n)) else selName'
--

-- Expansion of the above to avoid using the type family on the RHS
instance (KnownNat n, ProtoTypeName c) => GPrettyProtoSel (K1 i (Field n (OptionalField (Last (Value c))))) where
...
instance (KnownNat n, ProtoTypeName c) => GPrettyProtoSel (K1 i (Field n (OptionalField (Last (Enumeration c))))) where
  gprettyProtoSel _ selName' = text "optional" <+> protoTypeName (Proxy :: Proxy c) <+> text selName'' <+> text "=" <+> text (show (natVal (Proxy :: Proxy n))) <> char ';' where
    selName'' = if null selName' then "field" ++ show (natVal (Proxy :: Proxy n)) else selName'

instance (KnownNat n, ProtoTypeName c, Dataname (Rep c)) => GPrettyProtoSel (K1 i (Field n (OptionalField (Maybe (Message c))))) where
  gprettyProtoSel _ selName' = protoTypeName (Proxy :: Proxy c) $+$ text "optional" <+> dataname (Proxy :: Proxy (Rep c)) <+> text selName'' <+> text "=" <+> text (show (natVal (Proxy :: Proxy n))) <> char ';' where
    selName'' = if null selName' then "field" ++ show (natVal (Proxy :: Proxy n)) else selName'
--
